 ...
 */
bool XMLTag::getattr(std::string n, double & v) const {
 ...
 * attribute was found.
 */
bool XMLTag::getattr(std::string n, bool & v) const {
  AttributeMap::const_iterator it = attr.find(n);
  if ( it == attr.end() ) return false;
 ...
 * the corresponding value. @return false if no attribute was found.
 */
bool XMLTag::getattr(std::string n, long & v) const {
  AttributeMap::const_iterator it = attr.find(n);
  if ( it == attr.end() ) return false;
 ...
 * the corresponding value. @return false if no attribute was found.
 */
bool XMLTag::getattr(std::string n, int & v) const {
  AttributeMap::const_iterator it = attr.find(n);
  if ( it == attr.end() ) return false;
 ...
 * the corresponding value. @return false if no attribute was found.
 */
bool XMLTag::getattr(std::string n, std::string & v) const {
  AttributeMap::const_iterator it = attr.find(n);
  if ( it == attr.end() ) return false;
  v = it->second;
  return true;
 ...
 */
std::vector<XMLTag*> XMLTag::findXMLTags(std::string str,
          std::string * leftover) {
 ...
 */
void XMLTag::deleteAll(std::vector<XMLTag*> & tags) {
 ...
 */
void XMLTag::print(std::ostream & os) const {
  if ( name.empty() ) {
    os << contents;
    return;
  }
 ...
 */
TagBase::TagBase(const AttributeMap & attr, std::string conts)
 ...
 * attribute was found.
 */
bool TagBase::getattr(std::string n, double & v, bool erase) {
 ...
 */
bool TagBase::getattr(std::string n, bool & v, bool erase) {
  AttributeMap::iterator it = attributes.find(n);
  if ( it == attributes.end() ) return false;
  if ( it->second == "yes" ) v = true;
  if ( erase) attributes.erase(it);
 ...
 * attribute was found.
 */
bool TagBase::getattr(std::string n, long & v, bool erase) {
  AttributeMap::iterator it = attributes.find(n);
  if ( it == attributes.end() ) return false;
  v = std::atoi(it->second.c_str());
  if ( erase) attributes.erase(it);
 ...
 * attribute was found.
 */
bool TagBase::getattr(std::string n, int & v, bool erase) {
  AttributeMap::iterator it = attributes.find(n);
  if ( it == attributes.end() ) return false;
  v = int(std::atoi(it->second.c_str()));
  if ( erase) attributes.erase(it);
 ...
 * attribute was found.
 */
bool TagBase::getattr(std::string n, std::string & v, bool erase) {
  AttributeMap::iterator it = attributes.find(n);
  if ( it == attributes.end() ) return false;
  v = it->second;
  if ( erase) attributes.erase(it);
 ...
 */
void TagBase::printattrs(std::ostream & file) const {
 ...
 */
void TagBase::closetag(std::ostream & file, std::string tag) const {
  if ( contents.empty() )
    file << "/>\n";
 ...
 */
Generator::Generator(const XMLTag & tag)
 ...
 */
void Generator::print(std::ostream & file) const {
  file << "<generator";
 ...
 */
XSecInfo::XSecInfo(): neve(-1), maxweight(1.0), meanweight(1.0), negweights(false), 
      varweights(false) {}

/**
 * Create from XML tag
 */
XSecInfo::XSecInfo(const XMLTag & tag)
 ...
 */
void XSecInfo::print(std::ostream & file) const {
 ...
 * Intitialize default values.
 */
Cut::Cut(): min(-0.99*std::numeric_limits<double>::max()),
            max(0.99*std::numeric_limits<double>::max()) {}

/**
 * Create from XML tag.
 */
Cut::Cut(const XMLTag & tag,
 ...
 * Print out an XML tag.
 */
void Cut::print(std::ostream & file) const {
 ...
 */
bool Cut::match(long id1, long id2) const {
 ...
 */
bool Cut::passCuts(const std::vector<long> & id,
 ...
 */
double Cut::eta(const std::vector<double> & p) {
 ...
 */
double Cut::rap(const std::vector<double> & p) {
 ...
 */
double Cut::deltaR(const std::vector<double> & p1,
 ...
 */
bool Cut::outside(double value) const {
...
ProcInfo::ProcInfo(): iproc(0), loops(0), qcdorder(-1), eworder(-1) {}

/**
 * Create from XML tag.
 */
ProcInfo::ProcInfo(const XMLTag & tag)
  : TagBase(tag.attr, tag.contents),
 ...
 * Print out an XML tag.
 */
void ProcInfo::print(std::ostream & file) const {
 ...
 * Intitialize default values.
 */
MergeInfo::MergeInfo(): iproc(0), mergingscale(0.0), maxmult(false) {}

/**
 * Creat from XML tag.
 */
MergeInfo::MergeInfo(const XMLTag & tag)
  : TagBase(tag.attr, tag.contents),
 ...
 * Print out an XML tag.
 */
void MergeInfo::print(std::ostream & file) const {
 ...
 * Constructors
 */
WeightInfo::WeightInfo(): inGroup(-1), isrwgt(false),
  muf(1.0), mur(1.0), pdf(0), pdf2(0) {}

/**
 * Construct from the XML tag
 */
WeightInfo::WeightInfo(const XMLTag & tag)
  : TagBase(tag.attr, tag.contents),
 ...
 * Print out an XML tag.
 */
void WeightInfo::print(std::ostream & file) const {
 ...
 */
WeightGroup::WeightGroup(const XMLTag & tag, int groupIndex, std::vector<WeightInfo> & wiv)
  : TagBase(tag.attr) {
  getattr("type", type);
 ...
 */
Weight::Weight(): iswgt(false), born(0.0), sudakov(0.0) {}

/**
 * Create from XML tag
 */
Weight::Weight(const XMLTag & tag)
  : TagBase(tag.attr, tag.contents),
 ...
 * Print out an XML tag.
 */
void Weight::print(std::ostream & file) const {
  if ( iswgt )
...
// ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
   ...
   * Initialize default values.
   */
  Clus::Clus(): scale(-1.0), alphas(-1.0) {}

  /**
   * Initialize default values.
   */
  Clus::Clus(const XMLTag & tag)
   ...
   * Print out an XML tag.
   */
  void Clus::print(std::ostream & file) const {
    file << "<clus";
 ...
 */
void Scales::print(std::ostream & file) const {
 ...
 * Initialize default values.
 */
PDFInfo::PDFInfo(double defscale): p1(0), p2(0), x1(-1.0), x2(-1.0),
     xf1(-1.0),  xf2(-1.0), scale(defscale), SCALUP(defscale) {}

/**
 * Create from XML tag.
 */
PDFInfo::PDFInfo(const XMLTag & tag, double defscale)
  : TagBase(tag.attr, tag.contents),
 ...
 * Print out an XML tag.
 */
void PDFInfo::print(std::ostream & file) const {
  if ( xf1 <= 0 ) return;
  file << "<pdfinfo";
 ...
 * Assignment operator.
 */
HEPRUP & HEPRUP::operator=(const HEPRUP & x) {
 ...
 */
HEPRUP::HEPRUP(const XMLTag & tag, int versin)
 ...
 */
void HEPRUP::print(std::ostream & file) const {
 ...
 */
int HEPRUP::weightIndex(std::string name) const {
 ...
 * Initialize default values.
 */
EventGroup::EventGroup(): nreal(-1), ncounter(-1) {}
 ...
 * Assignment operator.
 */
HEPEUP & HEPEUP::operator=(const HEPEUP & x) {
  clear();
  setEvent(x);
 ...
 */
HEPEUP::HEPEUP(const XMLTag & tag, HEPRUP & heprupin)
 ...
 */
void HEPEUP::print(std::ostream & file) const {
 ...
 */
double HEPEUP::totalWeight(int i) const {
 ...
 */
double HEPEUP::totalWeight(std::string name) const {
 ...
 */
double HEPEUP::weight(int i) const {
 ...
 */
double HEPEUP::weight(std::string name) const {
 ...
 */
void HEPEUP::setWeight(int i, double w) {
  weights[i].first = w;
}
/**
 * Set the weight with the given name.
 */
bool HEPEUP::setWeight(std::string name, double w) {
 ...
 * weight will be used.
 */
bool HEPEUP::setWeightInfo(unsigned int i) {
 ...
 * will be chsen.
 */
bool HEPEUP::setSubEvent(unsigned int i) {
 ...
 */
bool Reader::currentFind(std::string str) const {
 ...
 */
std::ostream & Writer::headerBlock() {
  return headerStream;
}
 ...
 */
std::ostream & Writer::initComments() {
  return initStream;
}
 ...
 */
std::ostream & Writer::eventComments() {
  return eventStream;
}
